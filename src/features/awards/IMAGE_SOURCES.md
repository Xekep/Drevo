...
Для современного знака Росатома «Ветеран атомной энергетики и промышленности» пока не задан локальный визуал: самодельная реконструкция удалена, а случайные фотографии с аукционов не используются как источник. До появления проверенного изображения интерфейс показывает нейтральный fallback.

PNG создаются скриптом `ops/fetch-award-images.sh`: из исходной фотографии/скана удаляется только связанный с краем фон, затем изображение сохраняется как PNG32. Геометрия, надписи и детали самой награды не перерисовываются.
