...
При открытии профиля умершего птица сразу начинает один пролёт от верхней строки ФИО вверх и влево через область портрета. Наведение не требуется, поведение одинаково на компьютере и телефоне. SVG-контейнер — 64 px. Абсолютное позиционирование не добавляет отступов между фото и ФИО или между строками. Касания проходят сквозь птицу.

После загрузки атласа `requestAnimationFrame` запускает восемь фаз взмаха с частотой около 14 кадров/с. Общая длительность — 1,9 секунды; CSS отвечает за плавную траекторию и исчезновение. Компонент `components/memorial-name.tsx` обновляется только при смене кадра, остальная карточка не подписана на анимацию. Положение рисунков совмещено по голове. По окончании и при закрытии карточки планирование кадров прекращается; повторные наведения не запускают пролёт. При открытии другого человека компонент создаётся заново.

При `prefers-reduced-motion: reduce` показывается сидящая птица. Переключение настройки во время пролёта немедленно останавливает движение. До загрузки изображения и после завершения пролёта SVG скрыт.

Перемещение от начальной до конечной точки линейное на протяжении всего пролёта: промежуточных ключевых кадров положения и повторных разгонов/торможений нет. Масштаб уменьшается равномерно относительно головы птицы, прозрачность меняется независимо от траектории. Взмахи крыльев сохраняют постоянный интервал 70 мс.

Подпись для средств доступности — «Светлая память». Возраст не служит основанием считать человека умершим: нужна записанная дата или место смерти.
