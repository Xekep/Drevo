...
make_png() {
  ...
  # и не пытается перерисовывать саму награду. После trim возвращаем небольшой
  # прозрачный запас: иначе край медали визуально прилипает к границе PNG и
  # кажется обрезанным при масштабировании и drop-shadow в интерфейсе.
  convert "$prepared" \
    -alpha on \
    -fuzz "$fuzz" -fill none \
